...
SM_DIEESE <- function(dados) {
      ...
      lista_sm <- baixa_sm() %>%
            ...
            dplyr::mutate(Ano = as.numeric(trunc(as.numeric(competenciamov)/100)),
                          mes = trunc(as.numeric(substr(competenciamov, 5, 6))),
                          sm = salario_minimo,
                          .keep = "unused") %>%

            dplyr::filter(mes==2 | mes==5 | mes==8 | mes==11) %>%
            dplyr::mutate(Trimestre = (mes+1)/3,
                          .keep = "unused")


      dados_sm <- dados %>%
            dplyr::left_join(., lista_sm, by = c('Ano', 'Trimestre'))
